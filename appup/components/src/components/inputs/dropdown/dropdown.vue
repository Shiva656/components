...
export default {
  ...
  computed: {
    Modified_JSON_data: function () {
      ...
      if (this.search.length > 0) {
        searched_data = this.dropdownItems.filter(
          (opt) =>
            opt.text.toLowerCase().indexOf(this.search.trim().toLowerCase()) >
            -1
        );
      }
      //else returns the dropdown items to dropdown
      else {
        searched_data = this.dropdownItems;
      }
  ...
  methods: {
    ...
    convert_BootstrapVue_Options: function (data, key_value, key_label) {
