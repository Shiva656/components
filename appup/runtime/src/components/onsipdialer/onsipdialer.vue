...
<div>
   <!-- audio tones -->
      ...
      <audio id="ringtone"  :src="inboundTone" loop></audio>
      <audio id="ringbacktone" :src="outboundTone" loop></audio>
   <div v-if = "onSIPReady == true">
     <div class="btn-group-vertical ml-4 mt-4 py-4 px-3 dailer-pad bg-white shadow rounded mb-3 position-relative" role="group" aria-label="Basic example">
      ...
      <div v-if = "buttonPressed == true" class="input-group phone-pad-section px-3 pb-3 align-items-center mb-auto">
      ...
      <div v-if="callconnected == false || callforward == true || conferenceCall == true">
      ...
      <div class="d-inline-flex w-100 active-btns mt-3 dailed-btns justify-content-between">
        <button type="button" v-if="callconnected == false || callforward == true" :disabled="outboundTransfer == false" class="btn success-btn px-0 pt-0 small-icon w-100 rounded-circle" @click="digit_pressed('+')"><i class="fal fa-plus fa-sm"></i></button>
         <button type="button" v-if="callconnected == true && callforward == false"  :disabled="outboundTransfer == false" class="btn success-btn px-0 pt-0 small-icon w-100 rounded-circle" @click="hold_call()"><i v-if="callonHold==true" class="fas fa-play-circle"></i><i v-if="callonHold==false"  class="far fa-pause-circle"></i></button>
        <button v-if="callconnected == false || conferenceCall ==true" type="button" class="btn success-btn px-0 pt-0 w-100 rounded-circle" @click="make_call()" v-b-tooltip.hover title="Call"><i class="fal fa-phone fa-lg pl-1 text-success"></i></button>
        <button  v-else-if="callconnected == true && callforward == true" type="button" class="btn danger-btn px-0 pt-0 w-100 rounded-circle" @click="transfer_called()" v-b-tooltip.hover title="Call"><i class="fal fa-phone fa-sm pl-1 text-success"></i></button>
      ...
      <div v-if ="callconnected == true" class="mb-4 w-100">
        <div class="btn-group call-diconnect-btns small-icon w-100 justify-content-around">
          <button  type="button" class="btn btn-outline-secondary border-0 p-0 rounded-circle" :disabled= "true" @click="conference_call()" v-b-tooltip.hover title="Conference"><i class="fal fa-users"></i></button>
          ...
          <button type="button" class="btn btn-outline-secondary border-0 p-0 rounded-circle" @click="mute_call()" :disabled="outboundTransfer == false"><i v-if = "onsipMute == false" class="fal fa-microphone fa-sm" v-b-tooltip.hover title="Mute"></i><i v-if = "onsipMute == true" class="fal fa-microphone-slash fa-sm text-danger" v-b-tooltip.hover title="Unmute"></i></button>
        </div>
      </div>
      <div class="px-4 w-100 mt-3 mb-4 call-disconnect-btn" v-if="callconnected == true  && conferenceCall == false">
  ...
  <div v-else> OnSIP Device will be ready in few seconds, please wait .. </div>
</div>
</template>
<script>
import Vue from "vue";
...
export default {
  ...
  data() {
    return {
      ...
      onSIPReady: false,
      inComing:true,
      inboundTone:"https://appup-cdn.s3.amazonaws.com/static/incoming.ogg",
      outboundTone:"https://appup-cdn.s3.amazonaws.com/static/outgoing.ogg",
  ...
  mounted() {
    ...
    //Assign number to phoneNumber if number exist
    if(this.number){
      this.phoneNumber=this.number;
    }
    // After device is ready
    window.AppupPluginsManager.$on("onsip-registered", () => {
      this.onSIPReady = true;
    });

    // Incoming call
    window.AppupPluginsManager.$on("onsip-invite", event => {
        this.toneId = document.getElementById('ringtone');
        // Start the ringback tone
        this.startTone();
      // Message to ask when incoming call received
      let callMessage ="Do you want to accept new call from " + event.remoteIdentity.displayName;
      ...
      this.$dialog
        .confirm(callMessage, options)
        .then(function() {
          console.log("Call Accepted");
          _this.callconnected = true;
          _this.outgoing = 0;
          _this.phoneNumber = event.remoteIdentity.displayName;
          // set transfer call to true
          _this.outboundTransfer = true;
          _this.buttonPressed = true;
          // If the user accepts call
          window.AppupPluginsManager.exec("AppupOnSip", "answer", {});
        })
        .catch(function(e) {
          console.log("Call Rejected", e);

          //If the user rejects call
          window.AppupPluginsManager.exec(
            "AppupOnSip",
            "reject",
            event.session
          );
    ...
    window.AppupPluginsManager.$on("onsip-call-ringing", () => {
        // Start the ringback tone
        this.startTone();
    });

    window.AppupPluginsManager.$on("onsip-call-accepted", data => {
      // Stop the ringback tone
      this.stopTone();
      console.log("accepted data is", data);
      this.callconnected = true;
      this.outboundTransfer = true;
      this.callonHold=false;
      this.incrementTimer();
    });
    window.AppupPluginsManager.$on("onsip-call-rejected", () => {

    });
    window.AppupPluginsManager.$on("onsip-call-terminated", () => {
      // Stop the ringback tone
      this.stopTone();
  ...
  methods: {
    //When we press numbers on keypad
    digit_pressed(number) {
      ...
      if (this.callconnected == true) {
        ...
        window.AppupPluginsManager.exec("AppupOnSip", "dtmf", {
          dtmf: number
        });
      }
    },

    //Remove digits from keypad
    backSpace() {
    ...
    make_call() {
      ...
      this.outgoing = 1;
      this.callconnected = true;
      this.toneId= document.getElementById('ringbacktone');
      window.AppupPluginsManager.exec("AppupOnSip", "makeCall", {
        To: this.phoneNumber
      });
    },

    //Disconnect or Terminate the outbound call
    disconnect_call() {
      ...
      this.callconnected = false;
      if (this.outgoing == 1) {
        window.AppupPluginsManager.exec("AppupOnSip", "disconnect");
        clearInterval(this.timeInterval);
      } else if (this.outgoing == 0) {
        window.AppupPluginsManager.exec("AppupOnSip", "reject");
        clearInterval(this.timeInterval);
      } else {
    ...
    // Transfer Call
       ...
       transfer_called()
       {
          window.AppupPluginsManager.exec("AppupOnSip","transferCall",{'TransferNumber':this.phoneNumber});
       },
    /**
    ...
    incrementTimer() {
      ...
      this.timeInterval = setInterval(function() {
        ...
        document.getElementById("ongoingtimer").innerHTML =
          timestamp
            .getHours()
            .toString()
            .padStart(2, "0") +
    ...
    // Mute Call
    mute_call() {
      this.onsipMute = !this.onsipMute;
      window.AppupPluginsManager.exec("AppupOnSip", "muteCall", this.onsipMute);
    },
      // Hold Call
    hold_call() {
      this.callonHold = !this.callonHold;
      window.AppupPluginsManager.exec("AppupOnSip", "holdCall", this.callonHold);
    },
    // Sound methods
    startTone() {
      try{
        this.toneId.autoplay=true;
        this.toneId.load();
        //this.toneId.play();
     }
       catch(e){
...
.btn:hover {
  background: #f9f9f9 radial-gradient(circle, transparent 1%, #fff 1%)
    center/15000%;
}
