<template>
  <div class="bg-light" v-if = "twilioReady == true">
    ...
    <div class="btn-group-vertical ml-4 mt-4 py-4 px-3 dailer-pad bg-white shadow rounded mb-3 position-relative" role="group" aria-label="Basic example">
      ...
      <div v-if="callconnected == false || callforward == true">
      ...
      <div class="d-inline-flex w-100 active-btns mt-3 dailed-btns justify-content-between">
        ...
        <button v-if="callconnected == false" type="button" class="btn success-btn px-0 pt-0 w-100 rounded-circle" @click="make_call()" v-b-tooltip.hover title="Call"><i class="fal fa-phone fa-lg pl-1 text-success"></i></button>
...
export default{
    ...
    mounted(){
      ...
      window.AppupPluginsManager.$on("ringcentral-ready", (device) => 
      { 
        this.twilioReady = true;
      });
      
      // After twilio call connected
      window.AppupPluginsManager.$on("ringcentral-call-connected",(call) =>
      { 
      ...
      window.AppupPluginsManager.$on("ringcentral-call-disconnected", () =>
      {
        console.log("Call disconnected in Ringcentral");
      ...
      window.AppupPluginsManager.$on("twilio-incoming-call-cancelled", (conn) =>
      {
       ...
       // Incoming call 
      window.AppupPluginsManager.$on("ringcentral-incoming-call", (session) =>
      {
         // Message to ask when incoming call received
        // let callMessage = "Do you want to accept new call";
        let callMessage = "Do you want to accept new call "+session.request.from.uri.normal.user;
         ...
         let options = {
          ...
          this.$dialog.confirm(callMessage, options)
	        .then( function () {
             ...
             _this.phoneNumber = session.request.from.uri.normal.user;
             _this.outgoing = '0';

            _this.buttonPressed = true; 

              // If the user accepts call
              window.AppupPluginsManager.exec("AppupRingCentral", "answer",session);
	          })
	        .catch(function () {
              console.log("Call Rejected ");
              
              //If the user rejects call
               window.AppupPluginsManager.exec("AppupRingCentral", "reject")
          	});              
      });
     ...
     methods:{
        digit_pressed(number) 
        {
           ...
           if(this.callconnected == true)
           {
             ...
             window.AppupPluginsManager.exec("AppupRingCentral","dtmf",{id:this.callId,dtmf:number});
           }
        },
       make_call()
       {
          ...
          // Based on withCallMusic, call required function
           window.AppupPluginsManager.exec("AppupRingCentral", "makeCall", {'To' : this.phoneNumber}) ;
       }, 
       disconnect_call()
       {
          this.phoneNumber = "";
          this.outgoing = "";
          window.AppupPluginsManager.exec("AppupRingCentral", "disconnect");
       },
       backSpace() 
       {
